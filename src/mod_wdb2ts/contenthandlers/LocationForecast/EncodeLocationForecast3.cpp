...
const std::string EncodeLocationForecast3::metatemplate("<!--@@META@@-->");
...
EncodeLocationForecast3::
EncodeLocationForecast3( LocationPointDataPtr locationPointData_,
...
EncodeLocationForecast3::BreakTimeList::const_iterator
EncodeLocationForecast3::
			   ...
			   miutil::Indent &indent )
{
	     ...
	     it != fromtimes.rend(); ++it )
	{
		...
		if( aggregates.findPrecip( forecastProvider, *it, totime, precip ) ) {
			hasData = true;
			PrecipitationTags precipitationTag( precip.precip, precip.min, precip.max, precip.prob );
