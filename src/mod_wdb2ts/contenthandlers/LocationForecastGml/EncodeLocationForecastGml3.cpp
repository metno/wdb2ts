...
// PROJECT INCLUDES
#include <contenthandlers/LocationForecastGml/EncodeLocationForecastGml3.h>
#include <ptimeutil.h>
...
namespace {
   wdb2ts::RequestIterator dummyRequestIterator;
...
const std::string EncodeLocationForecastGml3::metatemplate("<!--@@META@@-->");
...
EncodeLocationForecastGml3::
EncodeLocationForecastGml3( RequestIterator &reqit_,
                            ...
                            int expire_rand_ )
   : reqit( reqit_ ), gmlContext( projectionHelper_, true ),
     tempCorrection( 0.0 ), from( from_ ),
     metaConf( metaConf_ ),
...
EncodeLocationForecastGml3::BreakTimeList::const_iterator
EncodeLocationForecastGml3::
...
encode(  webfw::Response &response )
{
	...
	miutil::TempFileStream tempFile;
 	...
 	if( reqit.polygon() ) {
 		try{
 		  WEBFW_LOG_DEBUG("POLYGON: true");
 			tmpost = tempFile.create( temporaryFilenamePrefix, reqit.app()->getTmpDir() );
 		}
 		catch( const exception &ex ) {
 			WEBFW_LOG_DEBUG("encode: Could not create temporary file in directory <" << reqit.app()->getTmpDir() << ">");
 			tmpost = &sost;
 		}
 	} else {
 	  WEBFW_LOG_DEBUG("POLYGON: false");
 		tmpost = &sost;
 	}
	...
	{ //Create a scope for the weatherdatatag and producttag.
		...
		while( locationPointData ) {
		   if( (*locationPointData).empty() ) {
		      WEBFW_LOG_DEBUG("LocationPointData: empty.");
		      locationPointData = reqit.next();
		      continue;
		   }

		   if( reqit.polygon() ) {
				longitude = (*locationPointData).begin()->first.longitude();
				latitude = (*locationPointData).begin()->first.latitude();

				try{
					altitude = reqit.app()->getHight( latitude, longitude );
				}
				catch( ... ) {
			...
			locationData.reset( new LocationData( (*locationPointData).begin()->second, longitude, latitude, altitude,
							                      reqit.providerPriority(), modelTopoProviders, topographyProviders ) );
	...
	if( contentLength > 0 ) {
		...
		miutil::StreamReplaceSource srs(ost, metatemplate, header, 1 );
		miutil::istreamreplace toSendStream( srs );

		if( filename.empty() ) {
		   WEBFW_LOG_DEBUG("Use sendStream");
			response.contentLength( contentLength );
			response.sendStream(  toSendStream );
		} else {
			filename += "_toSend";
			WEBFW_LOG_DEBUG("Use copyFromStreamToFile: filename:" << filename );
