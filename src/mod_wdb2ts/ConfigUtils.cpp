...
miutil::EnableTimePeriod configEnableThunderInSymbols(
		const wdb2ts::config::ActionParam &conf )
{
	WEBFW_USE_LOGGER( "handler" );
	miutil::EnableTimePeriod thunder;
	wdb2ts::config::ActionParam::const_iterator it = conf.find(
			"symbols_enable_thunder" );

	if( it == conf.end() ){
		WEBFW_LOG_INFO(
				"Config: No specification for 'symbols_enable_thunder' is given. Thunder is enabled throughout the year." );
		return thunder;
	}
	...
	if( !thunder.valid() ){
		WEBFW_LOG_ERROR(
				"Config: Invalid specification for 'symbols_enable_thunder', '" << val << "'. The format of the time period from/to is: MMM-DD/MMM-DD, where MMM is Jan, Feb, Mar, Apr, May, Jun, Jul, Aug, Sep, Oct, Nov and Dec. DD is the day in the month, 1 - 31. Thunder is NOT enabled." );
	}else{
		WEBFW_LOG_INFO(
				"Config: Thunder, 'symbols_enable_thunder', enabled for period: " << thunder << "." );
	}

	return thunder;
}

int configModelResolution( const wdb2ts::config::ActionParam &conf,
		int defaultResolutionInSeconds )
{
	WEBFW_USE_LOGGER( "handler" );
	wdb2ts::config::ActionParam::const_iterator it = conf.find(
			"model_resolution" );
	...
	if( it == conf.end() ){
		WEBFW_LOG_INFO(
				"Config: No specification for 'model_resolution'. Using default:" << defaultResolutionInSeconds << " seconds." );
	...
	try{
		...
		WEBFW_LOG_ERROR(
				"Config: Failed to read 'model_resolution'. " << ex.what() << ". Using default:" << defaultResolutionInSeconds << " seconds." );
	}
...
{
	WEBFW_USE_LOGGER( "handler" );
	wdb2ts::config::ActionParam::const_iterator it = conf.find("forecast");

	if( it == conf.end() ){
		WEBFW_LOG_INFO(
				"Config: 'forecast' not set, default true." );
		return true;
	}

	bool res = it->second.as<bool>(true);
...
bool NoDataResponse::doThrow() const
{
...
{
	switch( re.response ){
	case NoDataResponse::NotDefined:
		o << "NotDefined";
		break;
	case NoDataResponse::NotFound:
		o << "NotFound";
		break;
	case NoDataResponse::ServiceUnavailable:
...
bool OutputParams::paramFlag( std::string &param ) const
{
	bool flag = true;
...
bool OutputParams::setParam( const std::string &param, bool flag )
{
...
bool OutputParams::setParam( const std::string &param_ )
{
	string param( param_ );
...
void OutputParams::addParam( const std::string &param, bool f )
{
	defParams[param] = f;
}

bool OutputParams::useParam( const std::string &param ) const
{
...
OutputParams OutputParams::decodeOutputParams(
		const wdb2ts::config::ActionParam &conf )
{
	WEBFW_USE_LOGGER( "handler" );
	...
	for( vector<string>::const_iterator it = vals.begin(); it != vals.end();
			++it ){
		...
		if( !params.setParam( param ) ){
			WEBFW_LOG_ERROR(
					"output_params: Unknown parameter name '" << param << "'" );
		}
	}
