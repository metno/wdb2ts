...
namespace{
  bool   match(const std::string &ma, const std::string &pat,
...
dnmi::file::
Dir::
Dir(const std::string &dirname_)
  :dir(0), entry(0), casesensitive(true)
{
  open(dirname_);
}
...
dnmi::file::
Dir::
open(const std::string &dirname_,
     const std::string &p,
     bool  cs)
{
  DIR *tmp;
...
long 
dnmi::file::
fileSize(const std::string &name)
{
  struct stat s;
...
namespace{
  ...
  bool   
  lowerch(char c1, char c2, bool casesensitive)
  {
    if(casesensitive)
      return c1==c2;
